    ...
    **Has been directly copied from CUDA calculation centre, by just swapping cp to np, not optimized**
    ...
    def __init__(self, pos_grid, velocity, acceleration, k, sigma, l0, c, coord_change, ref_grid, divisor, pool_mass=10, g=np.array([0, 0, -9.81], dtype=np.float64), mega_array=True, timestep=0.1, debug=False, RAM=100000, integrator="ER"):
        ...
        if mega_array and not debug:
            mega_pos_grid = np.zeros((int((RAM)//pos_grid.nbytes), len(pos_grid[0]), len(pos_grid[1]), 3), dtype=np.float64)    #Defines size of mega_pos_grid, based on memory allocation
        ...
        self.shift_pos = self.quick_shift(self.pos_grid_cpu, coord_change, ref_grid, divisor, self.velocity_cpu)    # Creates shiftable array using Numba accelerated algorithm
    ...
    def runner(self, coord_change): # Host function, manages filling of the mega arrays
        self.kinetics_cpu = np.zeros((len(self.mega_pos_grid_cpu), len(self.mega_pos_grid_cpu[0]), len(self.mega_pos_grid_cpu[0, 0])))
        self.gpe_cpu = np.zeros((len(self.mega_pos_grid_cpu), len(self.mega_pos_grid_cpu[0]), len(self.mega_pos_grid_cpu[0, 0])))
        self.epe_cpu = np.zeros((len(self.mega_pos_grid_cpu), len(self.mega_pos_grid_cpu[0]), len(self.mega_pos_grid_cpu[0, 0])))
        self.frame = 0
    ...
    def hookes_law(self, coord_change):
        ...
        for repeat in range(8): #Repeat for every neighbour direction. With a bit of work could be cut in half by mirroring forces, however this will increase RAM requirements by around 10-20%.
            ...
            resultant_force_cpu += self.k_cpu * (modulus_cpu - self.l0_cpu[repeat]) * 1 / modulus_cpu * vector_difference_cpu + self.c_cpu * velocity_difference_cpu    # Combination of Hooke's law (F = k(x-l0)) and damping (F=cv). Note no minus due to directions of difference vectors
        self.resultant_force_cpu = (resultant_force_cpu + self.weight_arry_cpu) * self.ref_grid_cpu # Increase this timestep's resultant force, with Hooke's law, damping and weight


    def surface_tension(self):  # Finds force due to cohesive nature of fluids
        grad_x_cpu, grad_y_cpu = np.gradient(self.pos_grid_cpu[:,:,2])  # Vector gradients, to give vectors parallel to surface
    ...
    def verlet(self, coord_change, debug_verlet=False): # Simulation intergrator, chosen due to high energy conservation in kinematic situations, really Verlocity Verlet, based on https://www2.inp.uni-stuttgart.de/~inp/mediawiki/images/5/54/Skript_sim_methods_I.pdf
        ...
        if not debug_verlet:    # Used to specifically unit test Verlet with basic kinematics when debug_verlet = True
            self.hookes_law(coord_change)   # Force due to Hooke's law, weight and damping, done in this order as to make verlocity verlety work with velocity dependant forces, see https://www2.inp.uni-stuttgart.de/~inp/mediawiki/images/5/54/Skript_sim_methods_I.pdf
