...
# Define attribuytes for CUDA_Calculation_Centre.py and CPU_Calculation_Centre.py
k = float(10)
sigma = 0
...
calc_cpu = cpu.CPU_Calculations(Points, velocity, acceleration, k, sigma, l0, c, coord_change, ref_grid, divisor, 2.5 * 2.5, np.array([0, 0, 0], dtype=np.float64), True, deltaT, debug=True)
# Run hooke's law
position, energies = calc.runner(coord_change)
position_cpu, energies_cpu = calc_cpu.runner(coord_change)
...
for n, direction in enumerate(expected_values_resultant_force): # Check aghainst expected values
    if np.isclose(direction, calc_cpu.resultant_force_cpu[1, 1, n], rtol=0.01):
        outputs.append("within 1%")
    else:
        outputs.append("not within 1%")
print(f"\n----------------------------------------\nGPU Compute\n----------------------------------------\nFor the x direction, the simulated forces are {outputs[0]}, with a value of {calc.resultant_force_gpu[1, 1, 0]} as compared to the predicted value of {expected_values_resultant_force[0]}. This gives a percentage difference of {(calc.resultant_force_gpu[1, 1, 0] -expected_values_resultant_force[0])/ expected_values_resultant_force[0] * 100}%.\n\n"
      ...
      f"CPU Compute\n----------------------------------------\nFor the x direction, the simulated forces are {outputs[3]}, with a value of {calc_cpu.resultant_force_cpu[1, 1, 0]} as compared to the predicted value of {expected_values_resultant_force[0]}. This gives a percentage difference of {(calc_cpu.resultant_force_cpu[1, 1, 0] -expected_values_resultant_force[0])/ expected_values_resultant_force[0] * 100}%.\n\n"
      f"For the y direction, the simulated forces are {outputs[4]}, with a value of {calc_cpu.resultant_force_cpu[1, 1, 1]} as compared to the predicted value of {expected_values_resultant_force[1]}. This gives a percentage difference of {(calc_cpu.resultant_force_cpu[1, 1, 1] -expected_values_resultant_force[1])/ expected_values_resultant_force[1] * 100}%.\n\n"
      f"For the z direction, the simulated forces are {outputs[5]}, with a value of {calc_cpu.resultant_force_cpu[1, 1, 2]} as compared to the predicted value of {expected_values_resultant_force[2]}. This gives a percentage difference of {(calc_cpu.resultant_force_cpu[1, 1, 2] -expected_values_resultant_force[2])/ expected_values_resultant_force[2] * 100}%.\n\n----------------------------------------\n")
